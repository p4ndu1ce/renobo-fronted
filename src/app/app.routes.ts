...
export const routes: Routes = [
  ...
  // Solo rutas accesibles desde el bottom navbar: llevan main layout (header + bottom nav)
  {
    ...
    children: [
      ...
      { path: 'partner', loadComponent: () => import('./pages/placeholder/placeholder.component').then((m) => m.PlaceholderComponent), canActivate: [authGuard, roleGuard], data: { title: 'Panel Partner', roles: ['PARTNER'] } },
    ],
  },
  ...
  { path: 'chat', loadComponent: () => import('./pages/chat/chat-screen.component').then((m) => m.ChatScreenComponent), canActivate: [authGuard, roleGuard], data: { title: 'Chat', roles: ['CLIENT'] } },
